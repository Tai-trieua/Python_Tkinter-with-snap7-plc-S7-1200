...
class AccountPage:
    def __init__(self, AccountSystem_window):
        ...
        AccountSystem_window.resizable(0, 0)
                ...
                # window Icon
        icon = PhotoImage(file='images\\login.png')
        AccountSystem_window.iconphoto(True, icon)

        AccountSystem_window.title('DASHBOARD LOGIN')
        ...
        # ====== LOGO ==========
        logoIcon = Image.open('images\\login_64.png')
        photo = ImageTk.PhotoImage(logoIcon)
        logo = Label(landing_page, image=photo, bg='#ffffff')
        logo.image = photo
        logo.place(x=20, y=45)

        brand_name = Label(landing_page, text='DASHBOARD LOGIN', bg='#ffffff', fg='#ff6c38', font=("", 15, "bold"))
        brand_name.place(x=90, y=60)

        text = Label(landing_page, text="TaiTa Developer", bg='#ffffff', font=("", 12, "bold"))
        text.place(x=20, y=140)
        txt = "Welcome to the Dahsboard"
        text2 = Label(landing_page, text=txt,
                 fg="#6b6a69", bg='#ffffff', font=("", 11, "bold"))
        text2.place(x=20, y=180)

        # ====== LOGO ==========
        IotImage = Image.open('images\\internet-of-things_cloud_256.png')
        photo = ImageTk.PhotoImage(IotImage)
        IoT_image = Label(landing_page, image=photo, bg='#ffffff')
        IoT_image.image = photo
        IoT_image.place(x=30, y=370)
        ...
        brand_name = Label(sign_in, text='DASHBOARD LOGIN', bg='#ffffff', fg='#ff6c38', font=("", 15, "bold"))
        brand_name.place(x=90, y=60)

        text = Label(sign_in, text="TaiTa Developer", bg='#ffffff', font=("", 12, "bold"))
        text.place(x=20, y=140)

        txt2 = "Welcome to the Dahsboard"
        ...
        IotImage = Image.open('images\\internet-of-things_cloud_256.png')
        photo = ImageTk.PhotoImage(IotImage)
        IoT_image = Label(sign_in, image=photo, bg='#ffffff')
        IoT_image.image = photo
        IoT_image.place(x=30, y=370)
        ...
        def login_all():
            # Admin
            conn1 = sqlite3.connect('Database\IoT_Dashboard.db')
            ...
            # Guest
            conn2 = sqlite3.connect('Database\IoT_Dashboard.db')
            cursor2 = conn2.cursor()
            find_user2 = 'SELECT * FROM Guest_Acount WHERE guest_username = ? and guest_password = ?'
            cursor2.execute(find_user2, [(username_entry.get()), (password_entry.get())])

            # Employee
            conn3 = sqlite3.connect('Database\IoT_Dashboard.db')
            cursor3 = conn3.cursor()
            find_user3 = 'SELECT * FROM Employee_Acount WHERE employee_username = ? and employee_password = ?'
        ...
        def forgot_password():
            ...
            def change_password():
                db = sqlite3.connect('Database\IoT_Dashboard.db')
                cur = db.cursor()
                ...
                #db.commit()
                #db.close()
                #messagebox.showinfo('Congrats', 'Password 
        ...
        brand_name = Label(sign_up, text='DASHBOARD LOGIN', bg='#ffffff', fg='#ff6c38', font=("", 15, "bold"))
        brand_name.place(x=90, y=60)

        text = Label(sign_up, text="TaiTa Developer", bg='#ffffff', font=("", 12, "bold"))
        text.place(x=20, y=140)
        txt3 = "Welcome to the Dahsboard "
        ...
        IotImage = Image.open('images\\internet-of-things_cloud_256.png')
        photo = ImageTk.PhotoImage(IotImage)
        IoT_image = Label(sign_up, image=photo, bg='#ffffff')
        IoT_image.image = photo
        IoT_image.place(x=30, y=370)
